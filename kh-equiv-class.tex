...
This definition bears a resemblance to the one in \Cref{sec:khlinearplan}, except that we ask the conditions hold for \emph{every plan} belonging to a set of indistinguishable plans $\plans$. This way, we obtain a result similar to the one in the previous section.


\subsection{A Second Approach: Adaptable Plans}

\begin{theorem}\label{th:mc-khp-nadapt-undecidable}
    The model-checking problem for $\PKhunc$ under the semantics from \Cref{def:semantics:non-adap} is undecidable.
\end{theorem}
...
\begin{theorem}\label{th:mc-khp-adapt-decidable}
    The model-checking problem for $\PKhunc$ under the semantics from \Cref{def:semantics:adap} is decidable. Moreover, if each $\plans\in\Unc$ is given as input as a deterministic FSA, the problem is in $\PTIME$.
\end{theorem}