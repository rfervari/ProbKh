...
In order to determine when an agent knows how to achieve a goal, we need to characterize those %sequences of actions (or plans)
plans that result appropriate for such a purpose.  The notion of \emph{strongly executable} provides such characterization and indicates that a plan is ``fail proof''. This notion, discussed already in e.g.~\cite{Wang15lori} was inspired by conformant planning (see e.g.~\cite{Smith&Weld98,Bonet2010}).

For an LTS $\model=\tup{\S,\Act,\ra,\V}$, elements of $\Act^*$ are called \emph{plans} (with $\epsilon$ being the empty plan).  
%
