...
\begin{definition}[Uncertainty-based \lts]\label{def:ults}
    ...
    \begin{definition}
       Let $\plans \subseteq \Act^*$ and $U,T,\set{s,t} \subseteq \S$, we write $s\reach{\plans} t$ whenever  $s \reach{\plan} t$ for some $\plan\in\plans$, and $U \reach{\plans} T$ whenever for all $s\in U$, $s\reach{\plans} t$ implies $t\in T$. 
    \end{definition}
