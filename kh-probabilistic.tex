...
reflect uncertainty. This is given by the set $\Unc$ below.
...
\begin{definition}\label{def:pltsu}
    ...
    pairwise disjoint non-empty sets of plans as in
    \Cref{def:ults}.
\end{definition}
...
\begin{example}
  ...
  and let $\modelex^1$ be the PLTSU extending the PLTS $\modelex$ with $\Unc_1$ as the agent's perception.
  %
  ...
  and let $\modelex^2$ be the PLTSU extending the PLTS $\modelex$ with $\Unc_2$ as the agent's perception.
  %
  Notice that
...
\begin{proof}
  ...
  where $\truthset{\model}{\init}=\{s_{\textsf{i}}\}$ and
  $F=\{{s\in\S}\mid{\fin\in\V(s)}\}$.  
  \bigpedro{cuidado!!! que no parece funcionar :-/ (falta, igual)}
