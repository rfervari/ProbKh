...
The following lemma is central to the proof of decidability of model
checking $\PKhadapt$ properties.

\begin{lemma}\label{lm:mc:core}
  Let $\model$ be a PLTS with initial state $\sini\in\S_\model$ and
  let $G\subseteq\S_\model$ a set of goal states.
  %
  Let $\plans\subseteq\Act^*$ be a regular language accepted by the
  live DFA $\dfa$ with initial state $\tini\in\S_\dfa$.  Define
  %
  $R_{G{\times}F} = \{{\rho\in\cexecf^{\model_{\sini}{\times}\dfa}} \mid \text{ for some }i\geq 0, \last(\rho[..i])\in{G{\times}F}\}$
  %
  (viz., the set of complete executions that reach both the goal and a
