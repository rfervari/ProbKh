...
\begin{example}\label{ex:running:plans-exec}
  ...
  by the $\plans_{\star}$-compatible strategy $\strat_5$ defined so that\pedro{pongo ``defined so that'' y no ``defined by'' porque la definici\'on dada es incompleta (aunque sea lo \'unico relevante).  Podr\'ia haber aceptado igua el ``defined by'' si me salvaba el rengl[on, pero no lo hace.}
  %
  \[
  \begin{array}{l}
...
\pedro{\textbf{\color{red}@Ra\'ul:} este comentario t\'ecnico est\'a tratando de atajar a aquel revisor que te rechazo la primera vuelta del paper con Andr\'es.  Fijate que te parece (o si te parece)}
...
\cref{eq:plans:goal:q}\textcolor{red}{, meaning she can adapt to the course of action in $\plans$ with the best chance of success.}\pedro{no entiendo bien a que va esto en rojo. El esp\'iritu de la frase era indicar algo t\'ecnico: que las probabilidades nos habilitan a vivir en una especie de lugar intermedio entre la cuantificaci\'on universal y la existencial}
...
\pedro{aqui no me puedo escapar de la necesidad de subindicar $\models$. Ver como/hasta donde generalr esto.  Charlarlo con Ra\'ul}
%
...
%
However, for an $\PKhadapt$ property $\varphi$ in which $\kh^q$ does
...
\begin{proof}
  ...
  %
  For any other execution, $\strat^\star$ can be definied in anyway.
...
\SetKwFunction{MinReachProb}{MeanReachProb}%
\begin{algorithm}[t]
  \small
  \setstretch{1.1}
  ...
  \Function{\Check{$\varphi$}}{
    \Switch{$\varphi$}{
      \lCase{$\Prop$}{\Return{$\{{s\in\S}\mid {\varphi\in\V(s)}\}$}}
      \lCase{${\neg\psi}$}{\Return{$\S\setminus{}$\Check{$\psi$}}}
      \lCase{${\psi\wedge\phi}$}{\Return{\Check{$\psi$}$\cap$\,\Check{$\phi$}}}
      \Case{$\kh^q(\psi,\phi)$}{
        $A:={}$\Check{$\psi$}\;
        $G:={}$\Check{$\phi$}\;
        \ForEach{$i\in I$}{
...
%
In particular, notice that in \cref{alg:ln:rettrue}, the function
returns the whole set $\S$ of states if the $i$-th DFA witnesses the
validity of $\kh^q(\psi,\phi)$ and, in \cref{alg:ln:retfalse}, it
returns the empty set if no DFA witnesses $\kh^q(\psi,\phi)$.
%
\pedro{est\'a bien poner el algoritmo o es mucho?}
