...
%%   %
  ...
  (properly) or exists some $1\leq i \leq\min(|\plan|,|\bar{\rho}|)$
  such that $\plan[i]\neq\bar{\rho}[i]$.

  If $\plan\leq\bar{\rho}$, then $\bar{\rho}[..|\plan|]=\plan$ and
  hence, by \Cref{def:plan:compat}, necessarily
  $\strat(\rho[..|\plan|])(\complete) = 1$.  By \cref{eq:def:prob:ii},
  this implies that $\Prob^\strat_s(\{\rho\complete\})=0$,
  contradicting $(\star)$.

  If instead $\plan[i]\neq\bar{\rho}[i]=a_i$ with
  $1\leq i \leq\min(|\plan|,|\bar{\rho}|)$, by \cref{eq:def:prob:ii},
  ...
  $s\reach{\plan[..i]}t$ for $i<|\plan|$, that is, there are $s_0$,
  ...
  $s_1$, \ldots $s_{|\plan|}$ such that $s=s_0$, $t=s_{|\plan|}$, and
  $s_k\reach{\plan[k{+}1]}\Dirac_{s_{k{+}1}}$ for all $0\leq k < |\plan|$.
  %
  We can construct a $\plan$-compatible strategy $\strat$
  such that, for all $0\leq k < |\plan|$,
  $\strat(s_0\,\plan[1]\,s_1\ldots s_{k{-}1}\,\plan[k]\,s_k)(\plan[k{+}1],\Dirac_{s_{k{+}1}})=1$,
  and, in adition, for
  $\rho = s_0\,\plan[1]\,s_1\ldots s_{|\plan|{-}1}\,\plan[k]\,s_{|\plan|}$,
...
\bigpedro{falta teorema indecidibilidad}
...
\bigpedro{\textcolor{red}{\textbf{s\'uper importante!!!!}} guarda que para esta parte la idea de SE \textcolor{red}{\textbf{s\'i}} juega un rol (si es que la cuantificaci\'on es ``for some'' en la Def. 5) porque pide que todos los planes sean SE a\'un cuando no llevan nunca al goal}
