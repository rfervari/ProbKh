...
\begin{algorithm}[tb]
  ...
  \textbf{Input}: PLTSU $\model$, with agent's perception as a family of live DFA $\{\dfa_i\}_{i\in I}$, $\PKhadapt$ property $\varphi$\\
  ...
  \begin{algorithmic}[1] %[1] enables line numbers
    ...
    \CASE{$\kh^q(\psi,\chi)$}
      \STATE \quad\quad \textbf{for each} {$i\in I$}
          ...
          \STATE \quad\quad\quad \textbf{if} {($\mathit{qmin}\geq q$)} {\Return{$\S$}}\label{alg:ln:rettrue}
          \STATE \quad\quad\quad \Return{$\emptyset$}\label{alg:ln:retfalse}
